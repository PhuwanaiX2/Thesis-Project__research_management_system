...
if (isset($_POST['delete'])) {
    ...
    $del_type = "DELETE FROM type_thesis WHERE typethesis_id = ?";
...
if (isset($_POST['ids'])) {
    ...
    if (count($idsArray) > 0) {
        ...
        $sql = "DELETE FROM type_thesis WHERE typethesis_id IN ($placeholders)";
...
if (isset($_POST['add_type_thesis'])) {
    $typename = $_POST['typethesis_name'];
    $checkDuplicateSql = "SELECT * FROM type_thesis WHERE typethesis_name = ?";
    ...
    if (mysqli_stmt_num_rows($checkStmt) > 0) {
        ...
        $insertQuery = "INSERT INTO type_thesis (typethesis_name) VALUES (?)";
...
if (isset($_POST['edit_type_thesis'])) {
    $thesis_id = $_POST["typethesis_id"];
    $thesis_type_name = $_POST["typethesis_name"];

    $checkDuplicateSql = "SELECT * FROM type_thesis WHERE typethesis_name = ?  AND typethesis_id != ?";
    $checkStmt = mysqli_prepare($conn, $checkDuplicateSql);
    ...
    if (mysqli_stmt_num_rows($checkStmt) > 0) {
        ...
        // ไม่มีข้อมูลซ้ำ ทำการเพิ่มข้อมูล
        $updateQuery = "UPDATE type_thesis SET typethesis_name = ? WHERE typethesis_id = ?";
