...
if (isset($_POST['delete'])) {
    ...
    $del_type = "DELETE FROM prefix WHERE prefix_id = ?";
...
if (isset($_POST['ids'])) {
    ...
    if (count($idsArray) > 0) {
        ...
        $sql = "DELETE FROM prefix WHERE prefix_id IN ($placeholders)";
...
if (isset($_POST['add_prefix'])) {
    $typename = $_POST['prefix_name'];
    $checkDuplicateSql = "SELECT * FROM prefix WHERE prefix_name = ?";
    ...
    if (mysqli_stmt_num_rows($checkStmt) > 0) {
        // มีข้อมูลที่ไม่สามารถใช้ prefix_name ซ้ำกันได้
        echo json_encode(array("status" => "error", "msg" => "ข้อมูลนี้มีอยู่แล้ว"));
    } else {
        // ไม่มีข้อมูลซ้ำ ทำการเพิ่มข้อมูล
        $insertQuery = "INSERT INTO prefix (prefix_name) VALUES (?)";
...
if (isset($_POST['edit_prefix'])) {
    $thesis_id = $_POST["prefix_id"];
    $thesis_type_name = $_POST["prefix_name"];

    $checkDuplicateSql = "SELECT * FROM prefix WHERE prefix_name = ?  AND prefix_id != ?";
    $checkStmt = mysqli_prepare($conn, $checkDuplicateSql);
    ...
    if (mysqli_stmt_num_rows($checkStmt) > 0) {
        ...
        // ไม่มีข้อมูลซ้ำ ทำการเพิ่มข้อมูล
        $updateQuery = "UPDATE prefix SET prefix_name = ? WHERE prefix_id = ?";
